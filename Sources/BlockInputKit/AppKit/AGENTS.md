...
- Preserve progressive store assumptions: render only `loadedBlockCount` blocks, load only the next visible batch from the loading row, keep complete save/export snapshots independent from editor row population, and append loaded batches without reloading the full collection.
